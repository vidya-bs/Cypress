...
it('Enter Workspace Details', function() {

 cy.visit('https://gcp-astrum-ui.apiwiz.io/#/login')
cy.get(':nth-child(4) > .mt-2').type('workspace12');
cy.get('.btn-generic-blue-sm').click();
cy.get('div.mt-1 > .neo-input').type('Abhinav1');
...
cy.getLocalStorage().then(cookies => {
    cy.log(cookies)
})
})
...
it('Test Response Headers', function(){
    cy.setLocalStorage("x-token-astrum", cookieObj.cookie)
...
it('Generate swagger inside a request', function() {
    cy.setLocalStorage("x-token-astrum", cookieObj.cookie)
